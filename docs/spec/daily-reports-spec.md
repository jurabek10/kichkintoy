...
> **API note (updated 2026-06-04): the app API is oRPC-only.** The REST-style `METHOD /path` endpoints in this spec are conceptual — the live API is typed oRPC procedures in domain contracts under [`shared/src/api/orpc/`](../../packages/shared/src/api/orpc/), composed by [`orpc-contract.ts`](../../packages/shared/src/api/orpc-contract.ts), and consumed on web via the typed `orpc` client + TanStack Query (this feature lives under `orpc.reports.*`). Each maps to `orpc.<domain>.<procedure>`. See [`../adding-a-feature.md`](../adding-a-feature.md).
