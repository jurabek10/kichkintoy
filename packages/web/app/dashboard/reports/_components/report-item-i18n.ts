...
/**
 * Daily-report items are stored as language-neutral tokens (e.g. itemType
 * "meal", title "breakfast", value "half") and translated to the *viewer's*
 * language here at render time. That's what keeps a report written by an
 * English-speaking teacher readable in Uzbek for the parent.
 *
 * Older reports were saved with translated display text instead of tokens, so
 * each lookup also accepts the legacy English label and maps it back to its
 * token — existing data reads correctly without a database migration.
 */

type T = TFunction<"reports">;

// Title slot tokens → their translation keys.
...
// Per-itemType: the translation namespace and the set of valid value tokens.
...
// Legacy English labels (lowercased) → token, for reports saved before tokens.
...
export function translateItemTitle(title: string | null, t: T): string {
  ...
  return title; // free text (e.g. a custom subject) — show as written
}

/** The item's recorded value ("Ate about half"…) in the viewer's language. */
export function translateItemValue(
  itemType: string,
  value: string | null,
  t: T,
): string {
