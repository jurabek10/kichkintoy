  ...
  return (
    <Pressable onPress={() => router.push(feature.route)} className="w-1/4 items-center gap-2">
      <View
        ...
        {(feature.key === 'messages' || feature.key === 'complaints') && unreadCount > 0 ? <View className="absolute -right-1.5 -top-1.5 h-5 min-w-5 items-center justify-center rounded-full border-2 border-card bg-coral-ink px-1"><Text className="text-[9px] font-extrabold leading-none text-white">{unreadCount > 99 ? '99+' : unreadCount}</Text></View> : null}
      </View>
