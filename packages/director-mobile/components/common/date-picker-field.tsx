...
export function DatePickerField({
  ...
  const fieldLabel = label ?? t('attendance.date');
  const todayText = todayLabel ?? t('attendance.today');
  ...
  return (
    <>
      <Pressable
        ...
        <View className="h-10 w-10 items-center justify-center rounded-xl" style={{ backgroundColor: MINT_BG }}>
          <Ionicons name="calendar-outline" size={20} color={MINT_INK} />
        </View>
        ...
        {isToday ? (
          <View className="rounded-full px-2.5 py-1" style={{ backgroundColor: MINT_BG }}>
            <Text className="text-[11px] font-bold" style={{ color: MINT_INK }}>
              {todayText}
            </Text>
          </View>
        ) : null}
        ...
        <View className="flex-1 justify-end bg-black/40">
          ...
          <View className="rounded-t-3xl bg-card" style={{ paddingBottom: insets.bottom + 12 }}>
            <View className="flex-row items-center justify-between border-b border-border px-4 py-3.5">
              <Text className="text-base font-bold text-foreground">{fieldLabel}</Text>
            ...
            <View className="mx-4 mt-3 flex-row gap-2">
              <Pressable
                ...
                <Text className="text-[15px] font-bold text-muted">{todayText}</Text>
              </Pressable>
              <Pressable
                onPress={confirm}
