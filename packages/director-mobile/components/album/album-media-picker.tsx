...
const MAX = 50;
let counter = 0;
const nextKey = () => `a${Date.now()}-${counter++}`;

/**
 * Photos & videos for an album: pick from camera or gallery, upload straight to
...
export function AlbumMediaPicker({
  ...
  value: AlbumMediaItem[];
  onChange: Dispatch<SetStateAction<AlbumMediaItem[]>>;
  ...
  async function add(fromCamera: boolean) {
    ...
    const picked: AlbumMediaItem[] = result.assets.map((asset) => ({
      key: nextKey(),
      uri: asset.uri,
  ...
  return (
    <Card className="gap-3">
      <View>
        <Text className="text-base font-extrabold text-foreground">{t('composer.media')}</Text>
