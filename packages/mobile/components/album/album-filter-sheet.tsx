...
/**
 * The parent's album board filter: narrow by class (only when the child's albums
 * span more than one) and by date — the same bottom-sheet the teacher board uses,
 * carrying the web parent page's class + period filters.
 */
      ...
      <Pressable className="flex-1 justify-end bg-black/40" onPress={onClose}>
        <Pressable className="rounded-t-3xl bg-card px-4 pb-9 pt-3" onPress={() => {}}>
          ...
          <ScrollView style={{ maxHeight: height * 0.62 }} showsVerticalScrollIndicator={false}>
            {/* Class only matters when the child's albums span more than one. */}
            {classOptions.length > 1 ? (
              <>
                <Text className="mb-1 mt-3 text-[13px] font-bold text-muted">{t('table.allClasses')}</Text>
                <OptionRow
