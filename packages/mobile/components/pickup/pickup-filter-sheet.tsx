  ...
  return (
    <Pressable
      ...
      className={cn(
        ...
        active ? 'border-sunshine-ink bg-sunshine-ink' : 'border-border bg-card',
      )}>
...
/** ‹ July 2026 › — steps the month the records list is scoped to. */
...
/**
 * Narrow the pickup records by month or day, status, and (when the parent has
 * more than one child) child — the parent's mobile stand-in for the web
 * month + status + child toolbar, in the teacher board's bottom-sheet shape.
 */
        ...
        <Pressable className="rounded-t-3xl bg-card px-4 pb-9 pt-3" onPress={() => {}}>
          ...
          <ScrollView style={{ maxHeight: height * 0.62 }} showsVerticalScrollIndicator={false}>
            ...
            {childOptions.length > 1 ? (
              <>
                <Text className="mb-1 mt-4 text-[13px] font-bold text-muted">{t('filters.allChildren')}</Text>
                <OptionRow
                  label={t('filters.allChildren')}
                  count={total}
                  active={childId === 'all'}
                  onPress={() => onChild('all')}
                />
                {childOptions.map((item) => (
                  <OptionRow
                    key={item.id}
                    label={item.name}
                    count={childCounts[item.id] ?? 0}
                    active={childId === item.id}
                    onPress={() => onChild(item.id)}
                  />
                ))}
              </>
            ) : null}
